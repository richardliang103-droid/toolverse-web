...
export const toolManifests: readonly ToolManifest[] = [
  ...
  },
  {
    ...
    inputs: [{
      ...
      extensions: [".doc", ".docm", ".docx", ".epub", ".pdf", ".ppt", ".pptx", ".rtf", ".csv", ".ods", ".odp", ".odt", ".xls", ".xlsb", ".xlsm", ".xlsx"],
      maxFiles: 1,
      maxSizeBytes: 50 * MB,
    }],
    outputs: [{ kind: "file", mimeTypes: ["text/markdown"], extensions: [".md"] }],
    supportsBatch: false,
    supportsWorkspace: true,
    supportsRecipe: false,
    supportsOffline: true,
    handoff: { canSend: false, canReceive: false, kinds: [] },
    suggestedNextTools: ["markdown-editor", "text-cleaner"],
    privacy: "local-only",
    privacyNote: "文件在瀏覽器內用 WASM 轉換，不會上傳；首次使用需下載轉換元件。",
  },
  {
