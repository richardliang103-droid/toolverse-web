  ...
  return <section className="workspace calendar-workspace page-shell" aria-label="月曆" suppressHydrationWarning>
    <div className="panel panel-tinted calendar-grid-panel">
      ...
      <div className="calendar-grid" role="grid" aria-label={`${year} 年 ${month} 月`} suppressHydrationWarning>
        {days.map((day) => {
          const holiday = day.inCurrentMonth ? holidayFor(day.date) : null;
    ...
    <div className="panel calendar-controls">
      ...
      <p className="key-note">週末與國定假日會標色，包含農曆節日（春節、端午、中秋）與政府公告的補假，資料來自行政院人事行政總處<a href="https://data.gov.tw/dataset/14718" target="_blank" rel="noopener noreferrer">政府行政機關辦公日曆表</a>，目前涵蓋 2026～2027 年。範圍以外的年份查不到資料就不會標色，需要時請另外查詢官方行事曆。</p>
    </div>
  </section>;
}
